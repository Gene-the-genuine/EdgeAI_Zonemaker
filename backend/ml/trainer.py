...
class ModelTrainer:
    ...
    def train_epoch(self, train_loader: DataLoader) -> Dict[str, float]:
        ...
        logger.info(f"훈련 에포크 시작 - 총 배치 수: {total_batches}")
        
        for batch_idx, batch in enumerate(train_loader):
            batch_start_time = time.time()
            
            try:
                # 진행률 표시 (10%마다 또는 매 배치)
                if (batch_idx + 1) % max(1, total_batches // 10) == 0 or batch_idx == total_batches - 1:
                    elapsed_time = time.time() - start_time
                    progress = (batch_idx + 1) / total_batches * 100
                    logger.info(f"훈련 진행률: {progress:.1f}% ({batch_idx + 1}/{total_batches}), "
                            f"경과 시간: {elapsed_time:.1f}초")
                
                # 빈 배치 체크
                if not batch or len(batch) == 0:
                    logger.warning(f"배치 {batch_idx + 1}: 빈 배치 건너뛰기")
                    continue
                ...
                if batch_idx % max(1, total_batches // 20) == 0:  # 5%마다
                    logger.info(f"배치 {batch_idx + 1}: 손실={loss.item():.4f}, "
                            f"처리시간={batch_time:.2f}초")
                
            except Exception as e:
                logger.warning(f"배치 {batch_idx + 1} 훈련 중 오류: {e}")
                continue
        ...
        logger.info(f"훈련 에포크 완료 - 처리된 배치: {num_batches}/{total_batches}, "
                    f"총 소요시간: {total_time:.1f}초, 평균 손실: {avg_loss:.4f}")
    ...
    def export_to_onnx(self, save_path: str, input_shape: Tuple = None):
        """(제거 예정) 외부 프로젝트에서 변환을 수행하도록 변경됨"""
        logger.warning("export_to_onnx 는 더 이상 지원되지 않습니다. 외부 변환 파이프라인을 사용하세요.")
...
def load_training_checkpoint(checkpoint_path: str) -> Optional[Dict]:
    """(제거 예정) 외부 변환 파이프라인에서만 사용되던 헬퍼."""
    logger.warning("load_training_checkpoint 는 더 이상 내부에서 사용되지 않습니다.")
    try:
